...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (d) { const p = JSON.parse(d); if (p.locs) setLocs(p.locs); if (p.aLoc) setALoc(p.aLoc); }
    } catch(e) {}
    setLoaded(true)
  }, [])
  ...
  useEffect(() => {
    if (!loaded) return
    try { localStorage.setItem('vietart-scorecard', JSON.stringify({locs,aLoc})) } catch(e) {}
  }, [locs, aLoc, loaded])
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="bg-gradient-to-br from-navy to-slate text-white p-5 rounded-b-2xl shadow-lg">
        <h1 className="text-xl font-extrabold tracking-tight">SITE SCORECARD</h1>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {locs.map(l => (
            <button key={l.id} onClick={()=>setALoc(l.id)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap border-2 transition-all ${
        ...
        <div className="flex border-b-2 border-navy mt-4 mb-4 overflow-x-auto no-scrollbar">
          {tabs.map(t => (
            <button key={t.id} onClick={()=>setTab(t.id)}
              className={`px-4 py-3 text-xs font-semibold whitespace-nowrap rounded-t-lg transition-all ${
        ...
        {tab==='survey' && (
          <div className="space-y-4 fade-in">
            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-blue">
              <h3 className="font-bold text-sm text-navy mb-3">A. Thông tin chung mặt bằng</h3>
                  ...
                  <div key={f.key} className={f.key==='address'||f.key==='condition' ? 'sm:col-span-2' : ''}>
                    ...
                    {f.type==='textarea' ? (
                      <textarea value={loc.survey[f.key]||''} onChange={e=>upSurvey(f.key,e.target.value)}
                        rows={2} className="w-full mt-1 px-3 py-2 border border-gray-200 rounded-lg text-sm bg-yellow-50 resize-y" />
                    ) : (
                      <input value={loc.survey[f.key]||''} onChange={e=>upSurvey(f.key,e.target.value)}
                        className="w-full mt-1 px-3 py-2 border border-gray-200 rounded-lg text-sm bg-yellow-50" />
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-teal">
              <h3 className="font-bold text-sm text-navy mb-3">B. Đếm lưu lượng (15 phút × 4 khung giờ)</h3>
                        ...
                        <tr key={si} className={si%2?'bg-gray-50':''}>
                          ...
                          {['w','b','c'].map(t=>
                            <td key={t} className="px-1 py-1">
                              <input type="number" value={loc.survey[`tf_${t}_${si}`]||''}
                                ...
                                className="w-14 px-2 py-1 border border-gray-200 rounded text-center text-sm bg-yellow-50" />
                            </td>)}
                          ...
                          <td className="px-1">
                            <input value={loc.survey[`tf_p_${si}`]||''} onChange={e=>upSurvey(`tf_p_${si}`,e.target.value)}
                              placeholder="%" className="w-12 px-1 py-1 border border-gray-200 rounded text-center text-sm bg-yellow-50" />
                          </td>
                        </tr>
                      )
                    })}
            ...
            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-orange">
              <h3 className="font-bold text-sm text-navy mb-3">C. Đối thủ cạnh tranh (300m)</h3>
              {[0,1,2,3,4].map(i=>(
            ...
            {SURVEY_SECTIONS.map(sec=>(
              <div key={sec.key} className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4"
                ...
                <h3 className="font-bold text-sm text-navy mb-3">{sec.title}</h3>
                {sec.fields.map(f=>(
            ...
            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-gold">
              <h3 className="font-bold text-sm text-navy mb-3">Đánh giá tổng quan</h3>
              {[['summary_gut','Cảm nhận đầu tiên (gut feeling)'],['summary_pros','3 ƯU ĐIỂM nổi bật'],
                ...
                <div key={k} className="mb-2">
                  <label className="text-xs text-gray-500 font-semibold">{l}</label>
                  <textarea value={loc.survey[k]||''} onChange={e=>upSurvey(k,e.target.value)}
                    rows={3} className="w-full mt-1 px-3 py-2 border border-gray-200 rounded-lg text-sm bg-yellow-50 resize-y" />
                </div>
              ))}
            </div>
          </div>
              ...
              return (
                <div key={g.id} className="card bg-white rounded-xl border border-gray-200 border-t-4 border-t-brand-blue overflow-hidden">
                  <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                    <h3 className="font-bold text-sm text-navy">{g.name}</h3>
                        ...
                        <div className="flex justify-between items-start gap-2 flex-wrap">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 flex-wrap">
                              <span className={`text-xs ${c.c?'font-bold text-brand-red':'text-gray-800'}`}>
                                {c.c&&'⚠ '}{c.n}
                              </span>
                              {c.c && <span className="text-[9px] bg-brand-red text-white px-1.5 py-0.5 rounded font-bold">CRITICAL</span>}
                              <span className="text-[9px] text-gray-400">w:{c.w}%</span>
                            </div>
                            <button onClick={()=>setGuide(guide===c.idx?null:c.idx)}
                              className="text-[10px] text-brand-blue mt-1 hover:underline">
                          ...
                          <div className="flex gap-1">
                            {[1,2,3,4,5].map(v=>(
                              <button key={v} onClick={()=>upScore(c.idx, v===loc.scores[c.idx]?null:v)}
                                className={`score-btn w-8 h-8 rounded-md text-xs font-bold ${
                                  loc.scores[c.idx]===v
          ...
          <div className="space-y-4 fade-in">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {activeResults.map((r,i) => (
                <div key={r.locId} className="bg-white rounded-xl p-4 text-center border-2 shadow-sm"
                  ...
                  <div className="text-xs font-bold text-navy mb-1">{r.name}</div>
                  <div className="text-3xl font-extrabold" style={{color:r.verdict?.color}}>
                    {(r.pct*100).toFixed(1)}%
                  </div>
                  <div className="text-xs font-bold mt-1" style={{color:r.verdict?.color}}>{r.verdict?.label}</div>
                  <div className="text-[9px] text-gray-400 mt-1">{r.answered}/{CRITERIA.length} tiêu chí</div>
                  {r.totalCrit>0 && <div className="text-[10px] text-brand-red font-bold mt-1">⚠ {r.totalCrit} Critical Fail</div>}
                </div>
              ))}
            </div>
            ...
            {activeResults.length > 0 && (
              <div className="card bg-white rounded-xl p-4 border border-gray-200">
                <h3 className="font-bold text-sm text-navy mb-3">Radar — So sánh theo nhóm</h3>
            ...
            {activeResults.length > 0 && (
              <div className="card bg-white rounded-xl p-4 border border-gray-200">
                <h3 className="font-bold text-sm text-navy mb-3">Tổng điểm theo Location</h3>
            ...
            {activeResults.length > 0 && (
              <div className="card bg-white rounded-xl border border-gray-200 overflow-hidden">
                <h3 className="font-bold text-sm text-navy p-4 pb-2">Bảng xếp hạng</h3>
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-navy text-white">
            ...
            {/* Legend */}
            <div className="card bg-white rounded-xl p-4 border border-gray-200">
              <h3 className="font-bold text-sm text-navy mb-2">Phân loại Go/No-Go</h3>
              {[
                ...
                <div key={v.l} className="flex items-center gap-3 py-1.5 border-b border-gray-100 last:border-0">
                  <span className="text-xs font-bold w-32" style={{color:v.c}}>{v.l}</span>
                  <span className="text-[10px] text-gray-500 w-16">{v.t}</span>
                  <span className="text-[10px] text-gray-600">{v.d}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab==='export' && (
          <div className="space-y-4 fade-in">
            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-green">
              <h3 className="font-bold text-sm text-navy mb-3">Tải báo cáo</h3>
              <p className="text-xs text-gray-500 mb-4">File CSV chứa đầy đủ dữ liệu Survey, Scoring & Dashboard cho tất cả Location đã chấm điểm.</p>
              <button onClick={exportCSV}
                className="w-full py-3 bg-brand-green text-white rounded-xl font-bold text-sm hover:opacity-90 transition">
                📥 Tải báo cáo CSV
              </button>
              <p className="text-[10px] text-gray-400 mt-2">Mở bằng Excel / Google Sheets / Numbers</p>
            </div>

            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-blue">
              <h3 className="font-bold text-sm text-navy mb-3">Chia sẻ link cho team</h3>
              ...
              <button onClick={()=>{navigator.clipboard?.writeText(window.location.href); alert('Đã copy URL!')}}
                className="w-full py-3 bg-brand-blue text-white rounded-xl font-bold text-sm hover:opacity-90 transition">
                🔗 Copy URL chia sẻ
              </button>
            </div>

            <div className="card bg-white rounded-xl p-4 border border-gray-200 border-t-4 border-t-brand-red">
              <h3 className="font-bold text-sm text-navy mb-3">Reset dữ liệu</h3>
              <button onClick={()=>{
                if(confirm('Xóa toàn bộ dữ liệu? Không thể hoàn tác.')){
                  localStorage.removeItem('vietart-scorecard'); location.reload()
                }}}
                className="py-2 px-4 border-2 border-brand-red text-brand-red rounded-lg text-xs font-semibold hover:bg-red-50 transition">
                🗑️ Xóa & bắt đầu lại
              </button>
            </div>
          </div>
